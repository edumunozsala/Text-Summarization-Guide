...
def get_train_data(training_dir, train_file, nsamples):
    ...
    # Load the validation dataset: text and summary
    #valid_df=pd.read_csv(valid_filenamepath, header=0, usecols=[0,1], 
    #               nrows=NUM_SAMPLES)
...
def get_datasets(encoder_inputs, decoder_outputs, train_frac, batch_size, seed):
    ...
    val_dataset = val_dataset.shuffle(val_enc_inputs.shape[0], reshuffle_each_iteration=True).batch(
                                    batch_size, drop_remainder=True)
...
def eval_step(inp, targ, enc_hidden):
    ...
    dec_hidden = enc_hidden
    dec_input = tf.expand_dims([tokenizer_outputs.word_index['<start>']] * args.batch_size, 1)
    result_ids = tf.expand_dims([tokenizer_outputs.word_index['<start>']] * args.batch_size, 0).numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gru_units', type=int, default=512, metavar='N',
                        help='Units of the Gru layer (default: 512)')
    parser.add_argument('--att_units', type=int, default=16, metavar='N',
                        help='Attention dimension (default: 16)')
...
#                        help='number of layers (default: 4)')
    ...
    config.RNN_UNITS = args.gru_units
    ...
    encoder.save_weights(os.path.join(args.sm_model_dir, 'encoder'), overwrite=True, save_format='tf')
    #encoder.save(os.path.join(args.sm_model_dir, 'encoder'), save_format='tf')
    #attention_layer.save_weights(os.path.join(args.sm_model_dir, 'attention'), overwrite=True, save_format='tf')
    decoder.save_weights(os.path.join(args.sm_model_dir, 'decoder'), overwrite=True, save_format='tf')
    #decoder.save(os.path.join(args.sm_model_dir, 'decoder'), save_format='tf')
